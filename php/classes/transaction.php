<?php
    ...
    $sql_resp = $mysqli->query ("SELECT `$ticker`, `$ticker_price`, `manyopen`, `$ticker_limit` FROM `Users` WHERE id='$id'");
    ...
    //$b = $mysqli->query ("SELECT `id`, `LKOH`, `LKOH_price`, `$ticker`, '$ticker_price' FROM `Users` WHERE id='$id'");
    ...
    if ($numbershears == 0 ||  $numbershears == ""){
        ...
        $mysqli->query("UPDATE `Users` SET `$ticker_price`='$ticker_avg_price_new',`$ticker`='$ticker_vol_new',`manyopen`='$avlb_money_new' WHERE id='$id'");
        ...
        // записываем данные сделки табл истории сделок
        $mysqli->query("INSERT INTO `Dealuser`(`idname`, `ticker`, `shareprice`, `numbershears`, `timedeal`, `pointticket`) VALUES ('$idname', '$ticker', '$shareprice', '$numbershears', '$timedeal', '$pointticket')");
        $mysqli->query("UPDATE `Users` SET `$ticker`='$ticker_vol_new',`manyopen`='$avlb_money_new' WHERE id='$id'");
        echo json_encode(getPortfolio());
    ...
    if ($pointticket == "buy") {
        if ($shareprice*$numbershears <= $avlb_money) {
            ...
            $avlb_money_new = $avlb_money-$shareprice*$numbershears;
            $mysqli->query("UPDATE `Users` SET `$ticker_price`='$ticker_avg_price_new',`$ticker`='$ticker_vol_new',`manyopen`='$avlb_money_new' WHERE id='$id'");
            //$mysqli->query("UPDATE `Users` SET `$ticker`='$numbershears', `$ticker_price`='$shareprice' WHERE `id`='$id'");
            echo json_encode(getPortfolio());
        } else {
            echo json_encode("Недостаточно средств на покупку");
        };
