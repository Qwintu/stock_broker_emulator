...
<div class="main_block mb-4">
    ...
    <table class="position_tab table table-striped table-hover">
      <thead class="">
        <tr class="header_op_tab"><th width="10%"></th></th><th width="12%">Цена</th><th width="16%">Изм. цены за день, %</th><th width="0%"></th><th width="15%">Средн. цена  позиции</th><th width="15%">Изм. позиции, руб</th><th width="18%">Размер позиции, руб</th><th width="13%">Размер позиции, акции</th></tr>
      </thead>
...
<div class="main_block">
    <table class="stock_market_tab table table-striped table-hover">
      ...
      <tbody class="">
        ...
        <tr id="AFLT" class=""><td></td><td class="ticker">AFLT</td><td class="name">Аэрофлот</td><td class="price">0</td><td class="change">0</td><td class="close_">0</td>
            <td>
                <input type="text" oninput="this.value = this.value.replace(/[^0-9.]/g, ''); this.value = this.value.replace(/(\.*)\./g, '$1');" class="form_input form-control-sm" placeholder="Акции">
            </td>
            <td>
...
<script type="text/javascript">
    ...
    let aflt_price = document.querySelectorAll('#AFLT .price');
    ...
    let aflt_change = document.querySelectorAll('#AFLT .change');
    ...
    let aflt_close = document.querySelectorAll('#AFLT .close_');
              ...
              .then(result=>{
                  ...
                  if (result == "Недостаточно средств на покупку" || result == "Недостаточно лимитов по акции" || result == "Введите количество акций больше нуля" || result == "Ошибка операции"){
                    ...
                    //   create_portf_tab(result);
                      alert(`Покупка ${vol} шт ${ticker} по цене ${price}`);
                  };
              });
            ...
            fetch("/transaction",{
              ...
              .then(result=>{
                  ...
                  if (result == "Недостаточно средств на покупку" || result == "Недостаточно лимитов по акции"|| result == "Введите количество акций больше нуля" || result == "Ошибка операции"){
                      ...
                      portf_tab_onload();
                    //   create_portf_tab(result);
                      alert(`Продажа ${vol} шт ${ticker} по цене ${price}`);
                  };
              });
    ...
    // let timerId;
    // timerId = setTimeout(function getQuotes() {
    //     timerId = setTimeout(getQuotes, 2000)
    // }, 2000)
    ...
    function getQuotes() {
        ...
        .then(result=>{
            ...
            aflt_price[0].innerHTML = result.AFLT[0];
            ...
            lkoh_change[0].innerHTML = result.LKOH[2];
            gazp_change[0].innerHTML = result.GAZP[2];
            sber_change[0].innerHTML = result.SBER[2];
            rosn_change[0].innerHTML = result.ROSN[2];
            tatn_change[0].innerHTML = result.TATN[2];
            gmkn_change[0].innerHTML = result.GMKN[2];
            aflt_change[0].innerHTML = result.AFLT[2];
            afks_change[0].innerHTML = result.AFKS[2];
            moex_change[0].innerHTML = result.MOEX[2];
            vtbr_change[0].innerHTML = result.VTBR[2];
            nlmk_change[0].innerHTML = result.NLMK[2];

            lkoh_close[0].innerHTML = result.LKOH[1];
            gazp_close[0].innerHTML = result.GAZP[1];
            sber_close[0].innerHTML = result.SBER[1];
            rosn_close[0].innerHTML = result.ROSN[1];
            tatn_close[0].innerHTML = result.TATN[1];
            gmkn_close[0].innerHTML = result.GMKN[1];
            aflt_close[0].innerHTML = result.AFLT[1];
            afks_close[0].innerHTML = result.AFKS[1];
            moex_close[0].innerHTML = result.MOEX[1];
            vtbr_close[0].innerHTML = result.VTBR[1];
            nlmk_close[0].innerHTML = result.NLMK[1];
            
            
            // делаем на кнопках точность, как в котировках
            const dig_after_point = x => ( (x.toString().includes('.')) ? (x.toString().split('.').pop().length) : (0) );
            const check_n = n => ( n > 2 ? (n) : (2) );
            ...
            for(let i=0; i<btns_buy.length; i++){
                ...
                let n = dig_after_point(a);
                n = check_n(n);
                but_buy.innerText = Math.round((a*1.005) * (Math.pow(10, n))) / (Math.pow(10, n));
            };
            ...
            for(let i=0; i<Object.keys(result).length; i++){
                ...
                if(row == null){
                    ...
                    let price_tick = Math.round(Number((result[tick][0])) * 100) / 100;   // сделать точность, как на кнопках sell/buy
